...
export function SetPasswordClient({ token, email, otpType }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // When the user arrives via /api/auth/verify redirect, the OTP params are in
    // the URL. We verify here (on submit) instead of on GET so that link-preview
    // bots cannot consume the one-time token before the user opens the link.
    if (token && email && otpType) {
      ...
      if (verifyError) {
        ...
        if (msg.includes('expired') || msg.includes('otp')) {
          setError("Havola muddati tugagan. Administrator yangi havola yaratishi kerak.");
        } else if (msg.includes('used') || msg.includes('invalid') || verifyError.status === 401) {
          setError("Havola allaqachon ishlatilgan yoki yaroqsiz. Administrator bilan bog'laning.");
        } else {
